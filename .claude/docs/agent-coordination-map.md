...
```
  ...
  game-designer art-dir  narr-dir  lead-programmer  qa-lead  audio-dir
     ...
     +--+--+     |     +--+--+  +--+--+--+--+--+   |        |
     |  |  |     |     |     |  |  |  |  |  |  |   |        |
    sys lvl eco  ta   wrt  wrld bgs    ai    tl ui qa-t    snd
                                 |
                             +---+---+
                             |       |
                          perf-a           analytics
  ...
  Additional Leads (report to producer/directors):
    ...
    security-engineer       -- Anti-cheat, exploits, data privacy
  ...
  Browser Implementation (reports to lead-programmer):
    browser-game-specialist  -- All HTML/CSS/JS: game loops, rendering, input, audio, mobile
      (no sub-specialists currently)
```

### Legend
```
sys  = systems-designer       bgs = browser-game-specialist
lvl  = level-designer         ai  = ai-programmer
eco  = economy-designer       tl  = tools-programmer
ta   = technical-artist       ui  = ui-programmer
wrt  = writer
wrld = world-builder
...
| lead-programmer | browser-game-specialist, ai-programmer, tools-programmer, ui-programmer |
...
| security-engineer | lead-programmer (secure patterns review) |
...
6. release-manager      -- Deploys to Vercel, monitors deployment
...
- lead-programmer (if asset pipeline is affected)
