...
regressions. For browser games, verification is manual and browser-based —
you verify behavior in Chrome DevTools, check console output, test on mobile
viewports, and document playtesting findings.
...
**Browser Verification Checklist (run before marking any story Done):**
1. Open Chrome DevTools → Console. Zero errors on load and during play.
2. Play through the mechanic manually — verify it behaves as designed.
3. DevTools → Application → Local Storage: verify state saves and loads correctly (if applicable).
4. DevTools → Toggle Device Toolbar: test at 390×844 (iPhone 14) and 412×915 (Pixel 7).
5. Verify touch targets are large enough, no horizontal overflow, no clipped content.

**What to verify for every game mechanic:**
